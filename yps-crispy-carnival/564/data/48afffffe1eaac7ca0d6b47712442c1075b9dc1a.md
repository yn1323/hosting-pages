...
Locator: getByText(/募集をつくりました|募集をつくり、スタッフに通知しました/).first()
Expected: visible
Timeout: 10000ms
...
Call log:
  ...
  - waiting for getByText(/募集をつくりました|募集をつくり、スタッフに通知しました/).first()
...
- region "Notifications, top (alt+T)":
  - status "同じシフト募集が存在します。確認してください。":
    - img
    - text: 同じシフト募集が存在します。確認してください。
