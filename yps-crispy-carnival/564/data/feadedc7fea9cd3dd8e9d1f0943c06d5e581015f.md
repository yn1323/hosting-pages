...
- Name: scenarios/legal-consent-flow.test.ts >> 法務同意フロー >> 未同意スタッフは提出時に同意してシフト希望を提出できる
- Location: e2e/scenarios/legal-consent-flow.test.ts:42:3

# Error details

```
Error: Command failed: npx convex run --no-push testing:seedLegalStaffSubmitScenario {"legalConsentState":"missing"} --preview-name pr-564-e2e
✖ Failed to run function "testing:seedLegalStaffSubmitScenario":
Error: [Request ID: cc85ad97a155b729] Server Error
Could not find function for 'testing:seedLegalStaffSubmitScenario'. Did you forget to run `npx convex dev`?
